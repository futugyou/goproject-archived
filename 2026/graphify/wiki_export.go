...
func (e *WikiExporter) generateCommunityArticle(graph *KnowledgeGraph, communityID int, nodeIDs []string, label string, communityLabels map[int]string, cohesion float64) string {
...
func (e *WikiExporter) generateGodNodeArticle(graph *KnowledgeGraph, node GraphNode, communityLabels map[int]string) string {
	var sb strings.Builder
	label := node.Id
	if node.Label != "" {
		label = node.Label
...
func (e *WikiExporter) generateIndex(communities map[int][]string, communityLabels map[int]string, godNodes []GodNode, totalNodes, totalEdges int) string {
	var sb strings.Builder
...
func (e *WikiExporter) getCrossCommunityLinks(graph *KnowledgeGraph, nodeIDs []string, ownCommunityID int, communityLabels map[int]string) []crossLink {
